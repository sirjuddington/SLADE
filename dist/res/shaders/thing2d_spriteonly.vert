...
void main()
{
	float sq_size = max(tex_size.x, tex_size.y);
	...
	vertex_out.tex_coord = vec2(
		0.5 + in_position.x * sq_size * half_padding / tex_size.x,
		0.5 - in_position.y * sq_size * half_padding / tex_size.y
	);
	vertex_out.alpha     = inst_alpha;

	gl_Position = mvp * vec4(inst_position + in_position * sq_size * half_padding, 0.0, 1.0);
}
