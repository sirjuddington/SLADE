	...
	int         ext_default)
{
	...
	wxFileDialog fd(
		...
		wxString::FromUTF8(ui::getStateString(ui::FILEDIALOG_LAST_DIR)),
	...
	if (fd.ShowModal() == wxID_OK)
	{
		...
		// Set last dir
		ui::saveStateString(ui::FILEDIALOG_LAST_DIR, info.path);
	...
	int         ext_default)
{
	...
	if (fd.ShowModal() == wxID_OK)
	{
		...
		ui::saveStateString(ui::FILEDIALOG_LAST_DIR, strutil::Path::pathOf(filename));
...
string filedialog::openDirectory(string_view caption, wxWindow* parent)
{
	...
	{
		// Set last dir
		ui::saveStateString(ui::FILEDIALOG_LAST_DIR, dialog_open.GetPath().utf8_string());
