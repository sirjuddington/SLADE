...
bool MemChunk::importFile(const wxString& filename, uint32_t offset, uint32_t len)
{
	...
	if (!file.IsOpened())
	{
		Log::error(S_FMT("MemChunk::importFile: Unable to open file %s", filename));
...
bool MemChunk::exportFile(const wxString& filename, uint32_t start, uint32_t size) const
{
	...
	if (!file.IsOpened())
	{
		Log::error(S_FMT("Unable to write to file %s", filename));
...
uint8_t* MemChunk::allocData(uint32_t size, bool set_data)
{
	...
	{
		Log::error(S_FMT("MemChunk: Allocation of %d bytes failed: %s", size, ba.what()));
