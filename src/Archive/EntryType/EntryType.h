...
{
public:
	EntryType(std::string_view id = "Unknown");
	~EntryType() = default;
	...
	const std::string& formatId() const { return format_->id(); }
...
private:
	...
	std::string extension_  = "dat";     // File extension to use when exporting entries of this type
	std::string icon_       = "default"; // Icon to use in entry list
	std::string editor_     = "default"; // The in-program editor to use (hardcoded ids, see *EntryPanel constructors)
	std::string category_   = "Data";    // The type 'category', used for type filtering
	int         index_      = -1;
	...
	vector<std::string> match_extension_;
	vector<std::string> match_name_;
	...
	vector<std::string> section_;       // The 'section' of the archive the entry must be in, eg "sprites" for entries
										// between SS_START/SS_END in a wad, or the 'sprites' folder in a zip
	vector<std::string> match_archive_; // The types of archive the entry can be found in (e.g., wad or zip)
};
