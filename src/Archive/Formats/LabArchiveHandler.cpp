...
// Filename:    LabArchiveHandler.cpp
// Description: ArchiveFormatHandler for LAB archives from Outlaws
//
...
// -----------------------------------------------------------------------------
bool LabArchiveHandler::open(Archive& archive, const MemChunk& mc)
{
	...
	for (uint32_t d = 0; d < num_lumps; d++)
	{
		...
		ui::setSplashProgress((static_cast<float>(d) / static_cast<float>(num_lumps)));
		...
		auto name = reinterpret_cast<const char*>(&mc[namedir_offset + name_offset]);

		// Create & setup lump
		auto nlump                  = std::make_shared<ArchiveEntry>(name, size);
		nlump->exProp("NameOffset") = static_cast<int>(name_offset);
...
// -----------------------------------------------------------------------------
string LabArchiveHandler::detectResType(ArchiveEntry* entry)
{
...
// Returns true if successful, false otherwise
// -----------------------------------------------------------------------------
bool LabArchiveHandler::write(Archive& archive, MemChunk& mc)
{
	...
	dir_offset += 16 * archive.numEntries();
	for (uint32_t l = 0; l < archive.numEntries(); l++)
	{
		entry                       = archive.entryAt(l);
		entry->exProp("NameOffset") = static_cast<int>(name_offset);
	...
	for (uint32_t l = 0; l < archive.numEntries(); l++)
	{
		...
		entry->exProp("NameOffset") = static_cast<int>(name);
...
// -----------------------------------------------------------------------------
bool LabArchiveHandler::isThisFormat(const MemChunk& mc)
{
	// Check data was given
	if (!mc.hasData())
		return false;
...
// -----------------------------------------------------------------------------
bool LabArchiveHandler::isThisFormat(const string& filename)
{
	...
	namedir_size = wxINT32_SWAP_ON_BE(namedir_size);
	if (static_cast<unsigned>(file.Length()) < (namedir_offset + namedir_size))
		return false;
