...
// Filename:    SiNArchiveHandler.cpp
...
// -----------------------------------------------------------------------------
//
// SiNArchiveHandler Class Functions
//
// -----------------------------------------------------------------------------
...
bool SiNArchiveHandler::open(Archive& archive, const MemChunk& mc)
{
	...
	{
		log::error("SiNArchiveHandler::open: Opening failed, invalid header");
	...
	for (uint32_t d = 0; d < num_entries; d++)
	{
		...
		{
			log::error("SiNArchiveHandler::open: SiN archive is invalid or corrupt (entry goes past end of file)");
...
// Returns true if successful, false otherwise
// -----------------------------------------------------------------------------
bool SiNArchiveHandler::write(Archive& archive, MemChunk& mc)
{
	// Clear current data
	mc.clear();
...
// -----------------------------------------------------------------------------
bool SiNArchiveHandler::isThisFormat(const MemChunk& mc)
{
	// Check given data is valid
	if (mc.size() < 12)
		return false;
