...
// Filename:    WadArchiveHandler.cpp
// Description: ArchiveFormatHandler for Doom engine WAD archives
//
...
// -----------------------------------------------------------------------------
//
// WadArchiveHandler Class Functions
//
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
bool WadArchiveHandler::isWritable()
{
...
void WadArchiveHandler::updateNamespaces(Archive& archive)
{
	...
	for (unsigned a = 0; a < archive.numEntries(); a++)
	{
		...
		else if (strutil::endsWith(entry->upperName(), "_END"))
		{
			...
			if (!found && ns_name == "f")
			{
				NSPair ns(archive.rootDir()->entryAt(0), entry);
				ns.start_index = 0;
				ns.end_index   = a;
				ns.name        = "f";
				namespaces_.push_back(ns);
...
// -----------------------------------------------------------------------------
bool WadArchiveHandler::hasFlatHack()
{
...
bool WadArchiveHandler::open(Archive& archive, const MemChunk& mc)
{
	...
	{
		log::error("WadArchiveHandler::openFile: File {} has invalid header", archive.filename());
...
// Returns true if successful, false otherwise
// -----------------------------------------------------------------------------
bool WadArchiveHandler::write(Archive& archive, MemChunk& mc)
{
	// Don't write if iwad
	if (iwad_ && iwad_lock)
	{
	...
	mc.clear();
	mc.seek(0, SEEK_SET);
	if (!mc.reSize(dir_offset + archive.numEntries() * 16))
	{
...
// Returns true if successful, false otherwise
// -----------------------------------------------------------------------------
bool WadArchiveHandler::write(Archive& archive, string_view filename)
{
	// Don't write if iwad
	if (iwad_ && iwad_lock)
	{
...
// -----------------------------------------------------------------------------
shared_ptr<ArchiveEntry> WadArchiveHandler::addEntry(
	Archive&                 archive,
	...
	string_view              add_namespace)
{
	...
	for (auto& ns : namespaces_)
	{
		if (strutil::equalCI(ns.name, add_namespace))
		{
			...
			return addEntry(archive, entry, ns.end_index++, nullptr);
		}
	}
	...
	for (auto& special_ns : special_namespaces)
	{
		...
		{
			addNewEntry(archive, special_ns.letter + "_start");
			addNewEntry(archive, special_ns.letter + "_end");
...
// -----------------------------------------------------------------------------
bool WadArchiveHandler::removeEntry(Archive& archive, ArchiveEntry* entry, bool set_deleted)
{
	// Check entry
...
// extension)
// -----------------------------------------------------------------------------
bool WadArchiveHandler::renameEntry(Archive& archive, ArchiveEntry* entry, string_view name, bool force)
{
	// Check entry
	if (!archive.checkEntry(entry))
		return false;
...
// -----------------------------------------------------------------------------
bool WadArchiveHandler::swapEntries(Archive& archive, ArchiveEntry* entry1, ArchiveEntry* entry2)
{
	// Check entries
...
// -----------------------------------------------------------------------------
bool WadArchiveHandler::moveEntry(Archive& archive, ArchiveEntry* entry, unsigned position, ArchiveDir* dir)
{
	// Check entry
	if (!archive.checkEntry(entry))
		return false;
...
vector<MapDesc> WadArchiveHandler::detectMaps(Archive& archive)
{
	...
	auto index               = 0;
	auto entry_count         = archive.numEntries();
	auto entry               = archive.rootDir()->sharedEntryAt(index);
	...
	while (entry)
	{
		...
		{
			// Get map info
			auto md = mapDesc(archive, archive.entryAt(index - 1));
			...
			entry = archive.rootDir()->sharedEntryAt(++index);
			continue;
		}
		...
		if (maplump_found && index > 0)
		{
			...
			auto header_entry = archive.rootDir()->sharedEntryAt(index - 1);
...
void WadArchiveHandler::detectIncludes(Archive& archive)
{
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		auto entries   = findAll(archive, opt);
		if (!entries.empty())
		{
				...
				while (!tz.atEnd())
				{
					if (tz.checkNC(tokens[i]))
					{
						...
						if (i == 5) // skip ')'
							tz.adv();
						opt.match_name = name;
						if (auto match = findFirst(archive, opt))
...
// -----------------------------------------------------------------------------
bool WadArchiveHandler::isThisFormat(const MemChunk& mc)
{
	// Check size
	if (mc.size() < 12)
		return false;
...
// -----------------------------------------------------------------------------
bool WadArchiveHandler::exportEntriesAsWad(string_view filename, const vector<ArchiveEntry*>& entries)
{
