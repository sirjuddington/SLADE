...
bool SImage::loadJediFONT(const uint8_t* gfx_data, int size)
{
	...
	// Setup variables
	offset_x_ = offset_y_ = 0;
	height_               = READ_L16(gfx_data, 6) * numchr;
	width_                = READ_L16(gfx_data, 4);
	has_palette_          = false;
	type_                 = PALMASK;
	format_               = nullptr;
	...
	for (int i = 0; i < height_; ++i)
	{
		for (int p = 0; p < width_; ++p)
		{
			...
			case 2: mask_[(i * width_) + p] = ((READ_B16(gfx_data, o + (i * 2)) >> (15 - p)) & 1) * 255; break;
			case 3: mask_[(i * width_) + p] = ((READ_B24(gfx_data, o + (i * 3)) >> (23 - p)) & 1) * 255; break;
			case 4: mask_[(i * width_) + p] = ((READ_B32(gfx_data, o + (i * 4)) >> (31 - p)) & 1) * 255; break;
			default:
				clearData();
...
bool SImage::loadJaguarSprite(const uint8_t* header, int hdr_size, const uint8_t* gfx_data, int size)
{
	...
	// Setup variables
	int16_t ofsx, ofsy;
	width_       = READ_B16(header, 0);
	height_      = READ_B16(header, 2);
	...
	{
		Global::error =
			S_FMT("Invalid Jaguar sprite: header too small (%d) for column offsets (%d)", hdr_size, (8 + (width_ * 6)));
		return false;
	}
