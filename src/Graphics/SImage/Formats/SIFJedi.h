...
private:
	...
	unsigned getImageInfo(SImage::Info& info, MemChunk& mc, int index, JediBMHeader* header, bool& transparent)
	{
		...
		if (multibm)
		{
			...
			if (offset_offset + 4 > (int)mc.size())
			{
				global::error = "Jedi BM file: invalid multi-BM file";
				return 0;
			}
			const uint32_t*        header_poffset = (uint32_t*)(mc.data() + offset_offset);
			uint32_t               header_offset  = wxINT32_SWAP_ON_BE(*header_poffset) + 34;
			const JediBMSubheader* subheader      = (JediBMSubheader*)(mc.data() + header_offset);
...
protected:
	...
	{
		SImage::Info          info;
		const JediFMEHeader1* header1 = (JediFMEHeader1*)(data.data() + offset);
		const JediFMEHeader2* header2 = (JediFMEHeader2*)(data.data() + wxINT32_SWAP_ON_BE(header1->head2));
...
public:
	...
	SImage::Info info(MemChunk& mc, int index) override
	{
		// Read headers
		SImage::Info          info;
		const JediFMEHeader1* header1 = (JediFMEHeader1*)(mc.data());
		const JediFMEHeader2* header2 = (JediFMEHeader2*)(mc.data() + wxINT32_SWAP_ON_BE(header1->head2));
...
private:
	...
	{
		const JediWAXHeader* header = (JediWAXHeader*)data.data();
		...
		for (int i = 0; i < 32; ++i)
		{
			...
			if (waxi > sizeof(header) && waxi < data.size() - sizeof(JediWAXSubheader))
			{
				const JediWAXSubheader* subhdr = (JediWAXSubheader*)(data.data() + waxi);
				for (int j = 0; j < 32; ++j)
				{
					uint32_t seqj = wxINT32_SWAP_ON_BE(subhdr->seqs[j]);
					if (seqj > sizeof(header) && seqj < data.size() - sizeof(JediWAXSequence))
					{
						const JediWAXSequence* seq = (JediWAXSequence*)(data.data() + seqj);
						for (int k = 0; k < 32; ++k)
						{
							uint32_t framk = wxINT32_SWAP_ON_BE(seq->frames[k]);
							if (framk > sizeof(header) && framk < data.size() - sizeof(JediWAXSequence))
							{
								const JediFMEHeader1* frame = (JediFMEHeader1*)(data.data() + framk);
								uint32_t              cell  = wxINT32_SWAP_ON_BE(frame->head2);
								if (cell > sizeof(header) && cell < data.size() - sizeof(JediFMEHeader2))
								{
									bool notfound = true;
...
public:
	...
	SImage::Info info(MemChunk& mc, int index) override
	{
		...
		// Read headers
		const JediFMEHeader1* header1 = (JediFMEHeader1*)(mc.data() + offsets[info.imgindex]);
		const JediFMEHeader2* header2 = (JediFMEHeader2*)(mc.data() + wxINT32_SWAP_ON_BE(header1->head2));
