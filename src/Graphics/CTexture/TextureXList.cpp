...
bool TextureXList::removeDupesFoundIn(TextureXList& texture_list)
{
	...
	for (unsigned a = 0; a < textures_.size(); a++)
	{
		...
		if (other_texture_index < 0)
		{
			...
			log::info("KEEP Texture: {}. It's NOT in the other list.", this_texture->name());
			continue;
		}
		...
		{
			log::info("DELETE Texture: {}. It's FOUND in the other list and IS identical.", this_texture->name());
			indices_to_remove.push_back(a);
		}
		else
		{
			log::info("KEEP Texture: {}. It's FOUND in the other list but IS NOT identical.", this_texture->name());
		}
...
bool TextureXList::cleanTEXTURESsinglePatch(Archive* current_archive)
{
	...
	for (unsigned a = 0; a < textures_.size(); a++)
	{
		...
		{
			log::info("KEEP Texture: {}. It's not extended.", texture->name());
			continue;
		}
		...
		{
			log::info("KEEP Texture: {}. It has non-one number of patches.", texture->name());
			continue;
		}
		...
		if (!patch_entry)
		{
			log::info("KEEP Texture: {}. Its single patch {} failed to load.", texture->name(), patch->name());
			continue;
		}

		if (patch_entry->parent() != current_archive)
		{
			log::info("KEEP Texture: {}. Its single patch is from a different archive.", texture->name());
			continue;
		}
		...
		{
			log::info(
				"KEEP Textures: {} and {}. They are both using the same single patch {}.",
				texture->name(),
		...
		{
			log::info("KEEP Texture: {}. Its single patch has non-zero offsets.", texture->name());
			continue;
		}
		...
		{
			log::info("KEEP Texture: {}. Its single patch has different dimensions from the texture.", texture->name());
			continue;
		}
		...
		log::info("MAYBE DELETE Texture: {}. It's a basic single patch texture.", texture->name());
	...
	for (auto& a : archive_tx_list.textures_)
	{
		...
		for (int p = 0; p < texture->nPatches(); p++)
		{
			...
			{
				log::info(
					"KEEP Textures: {} and {}. They are both using patch {}.",
					texture->name(),
	...
	vector<unsigned int>           indices_to_remove;
	std::map<unsigned int, string> removal_messages;
	...
	for (auto iter : single_patch_textures)
	{
		...
		removal_messages[iter.second] = fmt::format(
			"DELETE Texture: {}. Convert Patch: {} to Texture File: {}.",
			texture->name(),
			patch_entry->name(),
			texture_file_name);
