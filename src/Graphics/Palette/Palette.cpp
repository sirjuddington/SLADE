...
bool Palette::loadMem(MemChunk& mc, Format format)
{
	...
	else if (format == Format::CSV || format == Format::JASC || format == Format::GIMP)
	{
		...
		do
		{
			...
			else if (format == Format::GIMP && !s1.Cmp("#")) { tz.skipLineComment(); continue; }
			...
			else if (format == Format::GIMP && !s2.Cmp(":")) { tz.skipLineComment(); continue; }
			...
			if (format == Format::GIMP) tz.skipLineComment();
