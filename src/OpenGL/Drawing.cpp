...
ColRGBA outline_colour     = ColRGBA{ 0, 0, 0, 255 };
...
{
// -----------------------------------------------------------------------------
// Loads all needed fonts for rendering
// -----------------------------------------------------------------------------
int initFonts()
{
	// --- Load general fonts ---
...
Vec2d drawing::textExtents(const string& text, Font font)
{
	...
	auto bbox = ftgl_font->BBox(text.c_str(), -1);
	return { bbox.Upper().X() - bbox.Lower().X(), ftgl_font->LineHeight() };
}
...
// Draws the text box
// -----------------------------------------------------------------------------
void TextBox::draw(int x, int y, const ColRGBA& colour, drawing::Align alignment) const
{
	Rectd b;
