...
// -----------------------------------------------------------------------------
SDataViewCtrl::SDataViewCtrl(wxWindow* parent, long style) :
	wxDataViewCtrl{ parent, -1, {}, {}, style },
	column_resize_check_timer_{ this }
{
	...
	column_resize_check_timer_.Start(250);
#endif
	...
	for (unsigned i = 0; i < GetColumnCount(); ++i)
	{
		...
		if (col->GetModelColumn() == col_model)
		{
			menu.AppendCheckItem(
					col_model, col->GetTitle(), WX_FMT("Show the {} column", col->GetTitle().utf8_string()))
				->Check(col->IsShown());
...
bool SDataViewCtrl::lookForSearchItemFrom(int index_start)
{
	...
	while (true)
	{
		...
		if (value.GetType() == wxS("string"))
			val_string = value.GetString();
		else if (value.GetType() == wxS("wxDataViewIconText"))
		{
			val_dvit << value;
			val_string = val_dvit.GetText();
		}

		if (strutil::startsWithCI(val_string.utf8_string(), search_))
		{
