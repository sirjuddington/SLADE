...
// -----------------------------------------------------------------------------
PaletteCanvas::PaletteCanvas(wxWindow* parent, int id) : GLCanvas(parent)
{
	// Bind events
...
void PaletteCanvas::draw()
{
	...
	const auto& shader = OpenGL::Draw2D::defaultShader(false);
	OpenGL::Draw2D::setupFor2D(shader, *this);

	// Draw palette
	vb_palette_.draw(OpenGL::Primitive::Quads);
}
...
// -----------------------------------------------------------------------------
void PaletteCanvas::setPalette(Palette* pal)
{
...
void PaletteCanvas::onMouseMotion(wxMouseEvent& e)
{
	...
	if (e.LeftIsDown() && allow_selection_ == SelectionType::Range)
	{
		...
		int x = e.GetX() / square_size_;
		int y = e.GetY() / square_size_;
